'use strict';
eSoafApp.controller('FPS910_DETAIL_1Controller',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "FPS910_DETAIL_1Controller";
		
		// combobox
		getParameter.XML(["FPS.INV_PRD_TYPE", "FPS.PORTFOLIO"], function(totas) {
			if (totas) {
		...
		$scope.init = function() {
			...
			//
			$scope.sendRecv("FPS910", "init_detail", "com.systex.jbranch.app.server.fps.fps910.FPS910InputVO", {'param_no': $scope.inputVO.param_no},
				function(tota, isError) {
					if (!isError) {
						// 查無資料
						if(tota[0].body.resultList.length == 0) {
							$scope.inputVO.totalList = [];
							$scope.inputVO.totalList.push({'CUST_RISK_ATR': 'C1-1','CIRCLE': 1,'SUBITEM': [{'INV_PRD_TYPE': '3','CIRCLE': 2,'SUBITEM': []}]}
						...
						$scope.inputVO.totalList = refreshDetail(tota[0].body.resultList);
					}
			});
		};
		$scope.init();
		...
		$scope.add = function () {
			...
			dialog.closePromise.then(function (data) {
				if(data.value != "cancel") {
					...
					var rootIndex = $scope.inputVO.totalList.map(function(e) { return e.CUST_RISK_ATR; }).indexOf(addData.cust_risk_atr);
					var childRow = $scope.inputVO.totalList[rootIndex].SUBITEM;
