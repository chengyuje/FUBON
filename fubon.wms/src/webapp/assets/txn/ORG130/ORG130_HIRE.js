...
//			   });
		...
		$scope.temp = function(row){
			...
			$scope.sendRecv("ORG130", "tempedit", "com.systex.jbranch.app.server.fps.org130.ORG130InputVO",$scope.inputVO,
					function(tota, isError) {
		...
		$scope.pass = function(){
			...
			$scope.sendRecv("ORG130", "passedit", "com.systex.jbranch.app.server.fps.org130.ORG130InputVO",$scope.inputVO,function(tota, isError) {
				if(!isError){
					if (isError) {
						$scope.showErrorMsg(tota[0].body.msgData);
		...
		$scope.unpass = function(row){
			...
			$scope.sendRecv("ORG130", "unpassedit", "com.systex.jbranch.app.server.fps.org130.ORG130InputVO",$scope.inputVO,
					function(tota, isError) {
		...
		$scope.transelse = function(row){
			$confirm({text: '是否轉介至他行：' + $scope.row.CUST_ID +'-'+ $scope.row.EMP_NAME + '進用資料?'}, {size: '200px'}).then(function() {
				 var dialog = ngDialog.open({
					template: 'assets/txn/ORG130/ORG130_TRANS.html',
					className: 'ORG130_TRANS',
					showClose: false, 
						...
						function(tota, isError) {
					 		if(!isError){  
								 if(tota[0].body.resultList4.length == 0) {
									 $scope.showMsg("ehl_01_org130_001");
									 return;
								 }
								 else {
