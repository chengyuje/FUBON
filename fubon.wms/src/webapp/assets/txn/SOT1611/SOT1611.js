 ...
 */
'use strict';
eSoafApp.controller('SOT1611Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, $interval, getParameter, $timeout) {
		...
		$scope.controllerName = "SOT1611Controller";
        ...
        $scope.noCallCustQuery = function () {
        	...
        	$scope.sendRecv("SOT1610", "query", "com.systex.jbranch.app.server.fps.sot1610.SOT1610InputVO", $scope.inputVO,
					function(tota, isError) {
						if (!isError) {
							if (tota[0].body.mainList.length == 0) {
								$scope.connector('set', "SOTTradeSEQ", null);
								$scope.connector('set', "SOTCarSEQ", null);
//								$scope.setSOTurl('assets/txn/SOT1610/SOT1610.html');
								$rootScope.menuItemInfo.url = "assets/txn/SOT1610/SOT1610.html";	
								return;
							}
...
//			$scope.applyBargainListTime = undefined;
		...
		$scope.delCar = function(row) {
			...
			$confirm({text: txtMsg}, {size: 'sm'}).then(function() {
				$scope.sendRecv("SOT1610", "delProd", "com.systex.jbranch.app.server.fps.sot1610.SOT1610InputVO", {tradeSEQ : $scope.inputVO.tradeSEQ},
						function(tota, isError) {
							if (!isError) {
...
//				$scope.setSOTurl('assets/txn/SOT1610/SOT1610.html');
		...
		$scope.do_goOP = function () {
			...
			if ($scope.inputVO.bargainFeeFlag == null || $scope.inputVO.bargainFeeFlag == '2') {
				...
				$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': '8', 'tradeType':1 ,
																													   'tradeSeq': $scope.inputVO.tradeSEQ}, 
				function(tota, isError) {
					if (!isError) {
						$scope.sendRecv("SOT1610", "goOP", "com.systex.jbranch.app.server.fps.sot1610.SOT1610InputVO", $scope.inputVO,
								function(tota, isError) {
									if (!isError) {
