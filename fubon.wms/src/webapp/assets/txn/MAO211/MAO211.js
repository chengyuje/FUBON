'use strict';
eSoafApp.controller('MAO211Controller', function ($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, getParameter, $filter, $q, sysInfoService) {
    $controller('BaseController', {$scope: $scope});
    $scope.controllerName = "MAO211Controller";
    ...
    $scope.inquire = function () {
    	$scope.sendRecv("MAO211", "inquire", "com.systex.jbranch.app.server.fps.mao211.MAO211InputVO", $scope.inputVO, function (tota, isError) {
    		if (!isError) {
    ...
    $scope.add_page = function () {
    	$scope.sendRecv("MAO211", "add_query", "com.systex.jbranch.app.server.fps.mao211.MAO211InputVO", $scope.inputVO, function (tota, isError) {
    		if (!isError) {
    			...
    			var dialog = ngDialog.open({
    				template: 'assets/txn/MAO211/MAO211_add.html',
    				className: 'MAO211_add',
    				showClose: false,
    			...
    			dialog.closePromise.then(function (list) {
    				if (list.value === 'cancel') {
    					...
    					if ($scope.relationshipList.length == 0) {
    						...
    						if (howmm > 6) {
    							$scope.showErrorMsg("ehl_01_MAO211_001");
    							$scope.canEdit = false;
    						}
    					} else {
    						// 主戶最多六個 2017/4/7
    ...
    $scope.applicationMethod = function () {
    	$scope.sendRecv("MAO211", "application", "com.systex.jbranch.app.server.fps.mao211.MAO211InputVO", $scope.inputVO, function (totas, isError) {
    		if (isError) {
