'use strict';
eSoafApp.controller('PMS101Controller', function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService,sysInfoService,$filter,getParameter) {
	...
	**===========================================================**/
    ...
    $scope.query = function() {
    	...
    	if ($scope.connector('get','CRM171_AO_CODE') == undefined) {
        	if ($scope.parameterTypeEditForm.$invalid) {				
	    		$scope.showErrorMsg('欄位檢核錯誤:年月、RM為必輸入欄位!!!');
    	...
    	if ($scope.shows == 'Y' && $scope.quertType == "Y") {
    		$scope.sendRecv("PMS101", "queryDataY", "com.systex.jbranch.app.server.fps.pms101.PMS101InputVO", $scope.inputVO, function(tota, isError) {
				if (!isError) {	
    	...
    	if ($scope.shows == 'N' && $scope.quertType == "Y") {
    		$scope.sendRecv("PMS101", "queryDataN", "com.systex.jbranch.app.server.fps.pms101.PMS101InputVO", $scope.inputVO, function(tota, isError) {
				if (!isError) {	
					$scope.alllist = tota[0].body.allList;
					$scope.outputVOy = angular.copy(tota[0].body);
					//未轉銷售陣列
    ...
    $scope.UpDate = function(inde){ 
		...
		$scope.sendRecv("PMS103", "queryData", "com.systex.jbranch.app.server.fps.pms103.PMS103InputVO", $scope.inputVO2, function(tota, isError) {
			if (!isError) {
