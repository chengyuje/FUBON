 ...
 */
'use strict';
eSoafApp.controller('IOT180Controller',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, sysInfoService,getParameter) {
		...
		$scope.controllerName = "IOT180Controller";
		...
		getParameter.XML(['IOT.REPORT_TYPE','POLICY_STATUS','IOT.SEND_TYPE','IOT.FIRST_PAY_WAY','IOT.WRITE_REASON',
		                  ...
		                  'CRM.PAY_WAY','CRM.CRM239_CONTRACT_STATUS','IOT.REPORT_AUTH_BRH','COMMON.YES_NO', 'IOT.CM_FLAG','IOT.PAYER_REL_PROPOSER',
		                  'IOT.REPORT_AUTH_BRH_ROLE','IOT.QC_APEC','IOT.QC_LOAN_CHK','IOT.QC_SIGNATURE'], function(tota) {
			if(tota){
				$scope.mappingSet['IOT.REPORT_AUTH_BRH'] = tota.data[tota.key.indexOf('IOT.REPORT_AUTH_BRH')];
				$scope.mappingSet['IOT.REPORT_AUTH_BRH_ROLE'] = tota.data[tota.key.indexOf('IOT.REPORT_AUTH_BRH_ROLE')];
...
//		    $scope.mappingSet['Report'].splice(4,1);  //匯率不show

			if($scope.mappingSet['IOT.REPORT_AUTH_BRH'][0]['LABEL'] == "Y"){
				for(var i = 0; i< $scope.mappingSet['IOT.REPORT_AUTH_BRH_ROLE'].length; i++){
					if(sysInfoService.getRoleID() == $scope.mappingSet['IOT.REPORT_AUTH_BRH_ROLE'][i]['LABEL']){
						$scope.auth = "Y";
					}
				}
        ...
        $scope.init = function(){	
			...
			if ($scope.memLoginFlag.startsWith('UHRM') && $scope.memLoginFlag != 'UHRM') {
				$scope.sendRecv("PMS401U", "isMainten", "com.systex.jbranch.app.server.fps.pms401u.PMS401UInputVO", {'itemID': 'IOT180'}, function(tota, isError) {
					if (!isError) {
						$scope.uhrmRCList = [];
						$scope.uhrmOPList = [];
...
//		var newDate = new Date(curDate);
		...
		*/
		...
		$scope.query = function(){
			if($scope.inputVO.ReportType == "5") { //匯率查詢
				$scope.queryExRate();
				return;
			}
			...
			$scope.sendRecv("IOT180", queryType , "com.systex.jbranch.app.server.fps.iot180.IOT180InputVO", $scope.inputVO,
			    function(tota, isError) {
					if (!isError) {
		...
		$scope.queryExRate = function(){
			$scope.sendRecv("IOT170", "queryExRate" , "com.systex.jbranch.app.server.fps.iot170.IOT170InputVO", $scope.inputVO,
			    function(tota, isError) {
					if (!isError) {			
						if(tota[0].body.resultList.length == 0){
							$scope.showMsg("ehl_01_common_009");
		...
		$scope.getDetail = function(row, type){			
			var tp, cn = '';				
			if(type == 'tar'){
				tp = 'assets/txn/IOT180/IOT180_INV_DETAIL.html';
				cn = 'IOT180_INV_DETAIL';
			}
			if(type == 'rider'){
				tp = 'assets/txn/IOT180/IOT180_RIDER_DETAIL.html';
				cn = 'IOT180_RIDER_DETAIL';
			}					
		...
		$scope.getRptDetail = function(row,val, brh,cls,type){
			...
			var dialog = ngDialog.open({
				template: 'assets/txn/IOT180/IOT180_REPORT_DETAIL.html',
				className: 'IOT180_REPORT_DETAIL',
				controller:['$scope', function($scope){
					$scope.row = row;
					$scope.type = type;
					$scope.brh = brh;
		...
		$scope.exportRPT = function(){
			$scope.sendRecv("IOT180", "export", "com.systex.jbranch.app.server.fps.iot180.IOT180OutputVO", 
