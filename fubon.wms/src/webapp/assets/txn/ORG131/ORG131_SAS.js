 ...
 */
'use strict';
eSoafApp.controller('ORG131_SASController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, sysInfoService, getParameter,$http) {
		...
		$scope.controllerName = "ORG131_SASController";
...
//	            $scope.chgStep(3);
	    ...
	    var uploadPdf = function (fileName, data) {
	      ...
	      //        pdfUrl: $sce.trustAsResourceUrl(url)
	      //      }, 'print');
	      //      return;
...
//	            generatePdfWithEncryption(tempFileName, fileName, data);
		...
		$scope.clear = function () {
            ...
            $scope.inputVO.evDate   =   new Date();//面談評估日期
...
//			if ($scope.isUpdate) {
//			  	  $scope.sendRecv("ORG131", "interviewHolding",	"com.systex.jbranch.app.server.fps.org131.ORG131InputVO",{
//					  branch_nbr : $scope.row.BRANCH_NBR,
//					  cust_id : $scope.row.CUST_ID
//				  },function(tota, isError) {
//							if (!isError) {
//								if(tota[0].body[0] != undefined	)	{
//									tmp = tota[0].body[0];
//
//								}
//							}
//				});
					$scope.row	=	tmp;
//			  	  console.log("init3:"+JSON.stringify($scope.row));
				$scope.setValue();
//
//			}
//	
		...
		$scope.save = function(row){
			...
			// 期望待遇NT$
//			if (!(typeof($scope.inputVO.expSalary) == 'number')) {				
//				if ($scope.inputVO.expSalary == '' || $scope.inputVO.expSalary == undefined || $scope.inputVO.expSalary == null) {
...
//			}
			...
			console.log(typeof($scope.inputVO.expectation)); 
            if ($scope.inputVO.hire_status != "4")
                $scope.inputVO.hireStatusTransRemark = "";
			$confirm({text: '是否儲存：' + $scope.row.CUST_ID +'-'+ $scope.row.EMP_NAME + '面談評估表?'}, {size: '200px'}).then(function() {
			$scope.sendRecv("ORG131", "saveInterviewHolding", "com.systex.jbranch.app.server.fps.org131.ORG131InputVO",$scope.inputVO,
					function(tota, isError) {
					if(!isError){
						if (isError) {
