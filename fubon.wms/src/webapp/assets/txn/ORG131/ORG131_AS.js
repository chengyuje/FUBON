 ...
 */
'use strict';
eSoafApp.controller('ORG131_ASController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, sysInfoService, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "ORG131_ASController";
		
		//組織連動繼承
		$controller('RegionController', {$scope: $scope});
		
		// filter
		getParameter.XML(["ORG.FINANCIAL_EXP", "ORG.PRE_FIN_INST", "ORG.REQ_CERTIFICATE"], function(totas) {
			if (totas) {
		...
		$scope.save = function(row){
			...
			$scope.sendRecv("ORG131", "interview", "com.systex.jbranch.app.server.fps.org131.ORG131InputVO",$scope.inputVO,
					function(tota, isError) {
					if(!isError){
						if (isError) {
