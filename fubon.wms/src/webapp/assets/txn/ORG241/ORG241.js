 ...
 */
'use strict';
eSoafApp.controller('ORG241Controller', function($scope, $controller, $confirm, $compile, socketService, ngDialog, projInfoService, $filter) {
	$controller('BaseController', {$scope : $scope});
	$scope.controllerName = "ORG241Controller";
	...
	$scope.query = function() {
		...
		$scope.sendRecv("ORG241", "query", "com.systex.jbranch.app.server.fps.org241.ORG241InputVO", $scope.inputVO, function(tota, isError) {
			if (isError) {
	...
	$scope.getExample = function() {
		$scope.sendRecv("ORG241", "getExample", "com.systex.jbranch.app.server.fps.org241.ORG241InputVO", $scope.inputVO, 
	...
	$scope.showORG241CONFIRM = function () {
		$scope.ORG241confirm_dialog = ngDialog.open({
			template: 'assets/txn/ORG241/ORG241CONFIRM.html',
			className : 'ORG241',
			showClose : false,
	...
	$scope.checkPoint = function(name, rname) {
		...
		$confirm({text: '是否上傳：' + rname + '?'}, {size: '200px'}).then(function() {
			$scope.sendRecv("ORG241", "checkPoint", "com.systex.jbranch.app.server.fps.org241.ORG241InputVO", $scope.inputVO, function(tota, isError) {
				if (isError) {
					$scope.showErrorMsgInDialog(tota.body.msgData);
					return;
				}
	...
	$scope.export = function() {
		...
		$scope.sendRecv("ORG241", "export", "com.systex.jbranch.app.server.fps.org241.ORG241InputVO", $scope.inputVO,
