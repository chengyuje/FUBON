'use strict';
eSoafApp.controller('MAO141Controller', function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $filter, getParameter, sysInfoService) {
	...
	$scope.init = function() {
        ...
        $scope.sendRecv("PMS401U", "isMainten", "com.systex.jbranch.app.server.fps.pms401u.PMS401UInputVO", {'itemID': 'MAO141'}, function(tota, isError) {
			if (!isError) {
	...
	$scope.inquire = function() {
		$scope.sendRecv("MAO141", "inquire", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", $scope.inputVO, function(tota, isError) {
			if (!isError) {
	...
	$scope.deletefile = function(){
    	...
    	$scope.sendRecv("MAO141", "delete", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", {'chkSEQ': ans}, function(totas, isError) {
			if (isError) {
	...
	$scope.lost = function(){
    	...
    	$scope.sendRecv("MAO141", "lost", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", {'chkSEQ': ans}, function(totas, isError) {
			if (isError) {
				$scope.showErrorMsgInDialog(totas.body.msgData);
				return;
            }
	...
	$scope.save = function(){
		...
		$scope.sendRecv("MAO141", "save", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", {'chkSEQ': ans}, function(totas, isError) {
			if (isError) {
				$scope.showErrorMsgInDialog(totas.body.msgData);
				return;
			}
	...
	$scope.showName = function(row){
		...
		$scope.sendRecv("MAO141", "showName", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", $scope.inputVO, function(tota, isError) {
			if (!isError) {
				debugger;
    ...
    $scope.exportfile = function() {
    	...
    	$scope.sendRecv("MAO141", "export", "com.systex.jbranch.app.server.fps.mao141.MAO141InputVO", $scope.inputVO, function(tota, isError) {
			if (isError) {
