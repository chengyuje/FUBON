'use strict';
eSoafApp.controller('PMS101UController', function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService,sysInfoService,$filter,getParameter) {
	
	$controller('BaseController', {$scope: $scope});
	
	$scope.controllerName = "PMS101UController";
	
	// 繼承
	$controller('PMS101Controller', {$scope: $scope});
	...
	$scope.initPMS_U = function() {
		...
		$scope.sendRecv("PMS401U", "isMainten", "com.systex.jbranch.app.server.fps.pms401u.PMS401UInputVO", {'itemID': 'PMS101U'},function(tota, isError) {
			if (!isError) {
