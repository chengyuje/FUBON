...
eSoafApp.controller('FPSProdSIController',
  function ($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, getParameter, $q) {
    ...
    $scope.inputVO = {};
    $scope.paramList = [];
    var detailPath = './assets/txn/PRD150/PRD150_DETAIL.html';
    // $scope.selection = []; 這個在主畫面
    // PRD.BOND_TYPE
    getParameter.XML(['PRD.SI_NEW_RATE_GUARANTEEPAY'], function (totas) {
      if (totas) {
        $scope.mapping.guaranteePay = totas.data[totas.key.indexOf('PRD.SI_NEW_RATE_GUARANTEEPAY')] || [];
      }
    });

    /* init */
    $scope.init = function () {
      $scope.inputVO.type = 'SI';
      $scope.inputVO.isRanked = 'Y';
      $scope.inputVO.riskType = $scope.riskType;
      $scope.inputVO.OBU = $scope.OBU;
      $scope.inputVO.isPro = $scope.isPro;
      $scope.inputVO.SIID = '';
      $scope.inputVO.SIName = '';
      ...
      $scope.inputVO.si_project = '';
      $scope.inputVO.si_customer_level = '';
      // $scope.prodList = [];
      $scope.outputVO = {};
    };
    ...
    $scope.getName = function () {
      ...
      if ($scope.inputVO.SIID) {
        $scope.sendRecv('PRD150', 'getSiName', 'com.systex.jbranch.app.server.fps.prd150.PRD150InputVO', {
            si_id: $scope.inputVO.SIID.toUpperCase()
          },
          function (tota, isError) {
            if (!isError) {
              if (tota[0].body.si_name) {
                $scope.inputVO.SIName = tota[0].body.si_name;
              }
              deferred.resolve();
            }
          });
    ...
    var initList = function () {
      $scope.prodList = angular.copy($scope.outputVO.prodList)
        .map(function (row) {
          row.name = row.PRD_ID + ' ' + row.SI_CNAME;
          // for main
