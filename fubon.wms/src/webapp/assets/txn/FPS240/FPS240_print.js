...
eSoafApp.controller('FPSPrintController',
  function ($scope, $controller, $timeout) {
    ...
    $scope.path = {
      ...
      validAmount: './assets/txn/FPS_PDF_Template/FPS240_component/validAmount.html',
      validAmount_ps: './assets/txn/FPS_PDF_Template/FPS240_component/validAmount_ps.html',

      firstPlanProduct: './assets/txn/FPS_PDF_Template/FPS240_component/firstPlanProduct.html',
      hasStockProduct: './assets/txn/FPS_PDF_Template/FPS240_component/hasStockProduct.html',

      firstPlanChart: './assets/txn/FPS_PDF_Template/FPS240_component/firstPlanChart.html',
      firstPlanChart_ps: './assets/txn/FPS_PDF_Template/FPS240_component/firstPlanChart_ps.html',
      hasStockChart: './assets/txn/FPS_PDF_Template/FPS240_component/hasStockChart.html',
      hasStockChartConclusion: './assets/txn/FPS_PDF_Template/FPS240_component/hasStockChartConclusion.html',

      historyPerformance: './assets/txn/FPS_PDF_Template/FPS240_component/historyPerformance.html',
      ...
      transaction: './assets/txn/FPS_PDF_Template/FPS240_component/transaction.html',
    ...
    var init = function () {
      ...
      $scope.blockObjects = [{
        title: '儲蓄型保險+存款',
        nowPct: $scope.depositNowPct,
        pct: $scope.depositPct,
        amt: $scope.depositAmt,
        list: $scope.depositList
      }, {
        title: '類債券商品',
      ...
      if($scope.custInfo.SIGN_AGMT_YN === 'N') {
    	  ...
    	  $scope.mapping.noticeTypes.push('FUND');
    	  $scope.mapping.noticeTypes.push('ETF');
    	  $scope.mapping.noticeTypes.push('BND');
    	  $scope.mapping.noticeTypes.push('SI');
    	  $scope.mapping.noticeTypes.push('SN');
      }
      // charts
      drawDoughnut();
      // rpt image
      ...
      // no data
      $scope.notShow = {
        historyPerformance: !($scope.yearRateList && $scope.yearRateList.length > 0) || $scope.volatility === '--',
        productPerformance: $scope.MFDPerformanceList.length <= 0 && $scope.DEBTPerformanceList.length <= 0,
...
//          console.log('SEN.SHEN');
    ...
    var amountDought = function (chartName, dataPoints, tableRoot, radius) {
      ...
      tableRoot[chartName][0].money = $scope.fixedAmt;
      tableRoot[chartName][1].money = $scope.stockAmt;
      tableRoot[chartName][2] = {};
	  tableRoot[chartName][2].label = "  待規劃金額";
	  tableRoot[chartName][2].money = $scope.headList.price - $scope.headList.cash - $scope.headList.ins - $scope.headList.cashPrepare - $scope.fixedAmt - $scope.stockAmt;
	  debugger
    };
    ...
    var watchCharts = $scope.$watch(function () {
      return document.getElementById('charts') != null && document.getElementById('yearRateChart');
    }, function (a, b) {
      init();
      if (a) {
        if (typeof (webViewParamObj) != 'undefined') 
