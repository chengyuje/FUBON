...
*/
'use strict';
eSoafApp.controller('FPS240PrintPreviewController',
  function ($scope, $controller, $http, ngDialog, projInfoService, $q, fps200Service, sysInfoService) {
    $controller('BaseController', {
      $scope: $scope
    });
    $scope.controllerName = "FPS240PrintPreviewController";
    ...
    var getData = function () {
      ...
      $scope.DEBTPerformanceList = $scope.data.DEBTPerformanceList;
    ...
    var getPrintTemplateURL = function () {
      return 'assets/txn/FPS_PDF_Template/FPS240_print_' +
        ($scope.hasIns ? 'has' : 'no') +
        'ins_commr' +
    ...
    $scope.init = function () {
      ...
      console.log('#0673測試: 看是哪一個PrintTemplateURL');
      console.log($scope.printTemplate);
      ...
      $scope.sendRecv('FPS240', 'inquire', 'com.systex.jbranch.app.server.fps.fps240.FPS240InputVO', {
        ...
        function (tota, isError) {
          if (!isError) {
            ...
            // 債券績效
            $scope.DEBTPerformanceList = tota[0].body.debtPerformanceList.map(function (row) {
                row.DATE_OF_MATURITY = row.DATE_OF_MATURITY ? Math.round(row.DATE_OF_MATURITY * 100) / 100 : '--';
//                row.FREQUENCY_OF_INTEST_PAY = row.FREQUENCY_OF_INTEST_PAY ? Math.round(row.FREQUENCY_OF_INTEST_PAY * 100) / 100 : '--';
...
//              //A.帳上資產
//              $scope.headList.liquidAmt = $scope.headList.deposit + $scope.headList.fundProd + $scope.headList.fixedProd;
//              //B.預計行外匯入金額
//              $scope.headList.sowAmt = $scope.headList.SOW_AMT || 0;
//              //小計A+B
//              $scope.headList.price = $scope.headList.liquidAmt + $scope.headList.sowAmt;
...
//              $scope.headList.stockPct = Math.round((100 - $scope.headList.depositPct - $scope.headList.fixedPct));
              ...
              $scope.headList.fixedAmt = $scope.hasIns ? Math.round($scope.headList.fixedAmt) : (Math.round($scope.headList.planAmt * $scope.headList.fixedPct / 100));
              $scope.headList.stockAmt = $scope.hasIns ? Math.round($scope.headList.stockAmt) : (Math.round($scope.headList.planAmt * $scope.headList.stockPct / 100));
              $scope.headList.fixedPct = $scope.headList.fixedPct;
              $scope.headList.stockPct = $scope.headList.stockPct;
              $scope.headList.deposit = $scope.headList.DEPOSIT_AMT || 0;
              $scope.headList.mfdProd = $scope.headList.MFD_PROD_AMT || 0;
              $scope.headList.etfProd = $scope.headList.ETF_PROD_AMT || 0;
              $scope.headList.insProd = 0;
              ...
              $scope.fixedAmt = Math.round($scope.fixedAmt);
              $scope.stockAmt = Math.round($scope.stockAmt);
