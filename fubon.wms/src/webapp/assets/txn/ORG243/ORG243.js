 ...
 */
'use strict';
eSoafApp.controller('ORG243Controller', function($scope, $controller, $confirm, $compile, socketService, ngDialog, projInfoService, $filter) {
	$controller('BaseController', {$scope : $scope});
	$scope.controllerName = "ORG243Controller";
	...
	$scope.query = function() {
		...
		$scope.sendRecv("ORG243", "query", "com.systex.jbranch.app.server.fps.org243.ORG243InputVO", $scope.inputVO, function(tota, isError) {
			if (isError) {
	...
	$scope.getExample = function() {
		$scope.sendRecv("ORG243", "getExample", "com.systex.jbranch.app.server.fps.org243.ORG243InputVO", $scope.inputVO, 
	...
	$scope.showORG243CONFIRM = function () {
		$scope.ORG243confirm_dialog = ngDialog.open({
			template: 'assets/txn/ORG243/ORG243CONFIRM.html',
			className : 'ORG243',
			showClose : false,
	...
	$scope.checkPoint = function(name, rname) {
		...
		$confirm({text: '是否上傳：' + rname + '?'}, {size: '200px'}).then(function() {
			$scope.sendRecv("ORG243", "checkPoint", "com.systex.jbranch.app.server.fps.org243.ORG243InputVO", $scope.inputVO, function(tota, isError) {
				if (isError) {
					$scope.showErrorMsgInDialog(tota.body.msgData);
					return;
				}
	...
	$scope.export = function() {
		...
		$scope.sendRecv("ORG243", "export", "com.systex.jbranch.app.server.fps.org243.ORG243InputVO", $scope.inputVO,
