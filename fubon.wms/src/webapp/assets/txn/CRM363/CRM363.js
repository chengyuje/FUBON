 ...
 */
'use strict';
eSoafApp.controller('CRM363Controller',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $q, getParameter, $filter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "CRM363Controller";

		// combobox
		getParameter.XML(["CRM.CON_DEGREE", "CRM.VIP_DEGREE", "COMMON.YES_NO", "CRM.TRS_APL_REASON", "CRM.CHG_TYPE", "CRM.TRS_PRJ_TYPE"], function(totas) {
			if (totas) {
		...
		$scope.getAllPRJ = function() {
			//若沒有選擇專案類型，先清掉專案名稱
			if($scope.checkNull($scope.inputVO.PRJ_TYPE)) $scope.inputVO.PRJ_ID = "";
			
			$scope.sendRecv("CRM363", "getAllPRJ", "com.systex.jbranch.app.server.fps.crm363.CRM363InputVO",$scope.inputVO,
					function(tota, isError) {
					   if(!isError){
						   $scope.TRS_PROJNAME = [];
		                	angular.forEach(tota[0].body.prj_list, function(row) {
		                		$scope.TRS_PROJNAME.push({LABEL: row.PRJ_NAME, DATA: row.PRJ_ID, PRJ_TYPE: row.PRJ_TYPE});
		                	});
		                	$scope.inputVO.trs_type = ($scope.inputVO.PRJ_TYPE == "2" ? "C" : "D");
					   }
				});
		};
		$scope.getAllPRJ();
		...
		//選擇專案名稱後，帶出專案類型
		$scope.prjIdChanged = function() {
			if($scope.checkNull($scope.inputVO.PRJ_TYPE)) {
				var findPrj = $filter('filter')($scope.TRS_PROJNAME, {DATA: $scope.inputVO.PRJ_ID});
				$scope.inputVO.PRJ_TYPE = (findPrj != null && findPrj.length > 0) ? findPrj[0]["PRJ_TYPE"] : "";
				$scope.inputVO.trs_type = ($scope.inputVO.PRJ_TYPE == "2" ? "C" : "D");
			}
		}
		...
		$scope.inquire = function() {
			...
			$scope.sendRecv("CRM363", "inquire", "com.systex.jbranch.app.server.fps.crm363.CRM363InputVO", $scope.inputVO,
				function(tota, isError) {
					if (!isError) {
...
//        	}
        	...
        	$scope.checkMax().then(function(res_data) {
				...
				} else if (res_data.resultList2 == 'ERR5') {
					$scope.showErrorMsg("客戶:" + res_data.resultList + ", 重新分派AO不可與原名單AO相同");
				} else if (res_data.resultList2 == 'ERR6') {
					$scope.showErrorMsg("客戶:" + res_data.resultList + ", 在ON CODE排除名單中，不可做移轉");					
				} else if (res_data.resultList2 == 'ERR7') {
					$scope.showErrorMsg("客戶:" + res_data.resultList + ", 區域分行重新分派AO不可與原名單AO相同");
		...
		$scope.save2 = function (flag) {
			...
			$scope.sendRecv("CRM363", "save", "com.systex.jbranch.app.server.fps.crm363.CRM363InputVO", $scope.inputVO2,
    			function(totas, isError) {
        			if (!isError) {
		...
		$scope.exportData = function(){
			...
			$scope.sendRecv("CRM363","export","com.systex.jbranch.app.server.fps.crm363.CRM363InputVO", 
					{'printList': $scope.resultList, 'printAllData' : false, 'PRJ_ID': $scope.inputVO.PRJ_ID, 'PRJ_TYPE': $scope.inputVO.PRJ_TYPE}, function(tota, isError) {
						if (isError) {							
							return;            		
		            	}
					});			
		...
		$scope.exportAllData = function(){
			...
			$scope.sendRecv("CRM363","export","com.systex.jbranch.app.server.fps.crm363.CRM363InputVO", 
					{'printList': $scope.paramList, 'printAllData' : true, 'PRJ_ID': $scope.inputVO.PRJ_ID, 'PRJ_TYPE': $scope.inputVO.PRJ_TYPE}, function(tota, isError) {
						if (isError) {							
							return;            		
		            	}
					});			
