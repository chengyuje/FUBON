 ...
 */
'use strict';
eSoafApp.controller('PMS324Controller',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService) 
	{
		$controller('BaseController', {$scope: $scope});
		//繼承共用的組繼連動選單
		$controller('PMSRegionController', {$scope: $scope});
		$scope.controllerName = "PMS324Controller";	
		$scope.init = function()
		{
			$scope.inputVO = {
					ao_code          : '',
		...
		$scope.isq = function(){
			 $scope.sendRecv("PMS324", "getDurExam", "com.systex.jbranch.app.server.fps.pms324.PMS324InputVO", $scope.inputVO,
        ...
        $scope.dateChange = function() {
			...
			$scope.sendRecv("PMS324", "getexeDurExam", "com.systex.jbranch.app.server.fps.pms324.PMS324InputVO", $scope.inputVO,
					function(totas, isError) {
	                	if (isError) {
	                		$scope.showErrorMsg(totas[0].body.msgData);
	                	}
      ...
      //選取計值月份
    	...
    	{	
    		...
    		var dialog = ngDialog.open({
    			template: 'assets/txn/PMS324/PMS324_upload.html',
    			className: 'PMS324_upload',
    	...
    	/**設定晉級門檻參數**/
    	$scope.parameter = function(sTime)
    	{
    		if($scope.parameterTypeEditForm.$invalid)
			{
	    		$scope.showErrorMsg('欄位檢核錯誤:請選擇必填欄位');
    		...
    		var dialog = ngDialog.open({
    			template: 'assets/txn/PMS324/PMS324_parameter.html',
    			className: 'PMS324_parameter',
    			showClose: false,
    			controller: ['$scope', function($scope) {
    				$scope.yearMon = sTime;
                }]
    	...
    	{
			$scope.sendRecv("PMS324", "export",	"com.systex.jbranch.app.server.fps.pms324.PMS324OutputVO",{
		...
		$scope.inquire = function()
		{
			...
			$scope.sendRecv("PMS324", "inquire", "com.systex.jbranch.app.server.fps.pms324.PMS324InputVO", $scope.inputVO,
					function(tota, isError) {
						if (!isError) 
						{
							$scope.paramList = [];
