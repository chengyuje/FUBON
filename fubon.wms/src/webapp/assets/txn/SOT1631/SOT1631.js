 ...
 */
'use strict';
eSoafApp.controller('SOT1631Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, $interval, getParameter, $timeout) {
		...
		$scope.controllerName = "SOT1631Controller";
		
		getParameter.XML(["SOT.CUST_TYPE", "SOT.CHANGE_TRADE_SUB_TYPE", "SOT.TRADE_DATE_TYPE", "COMMON.YES_NO", "SOT.TRUST_CURR_TYPE", "SOT.SPEC_CUSTOMER", "SOT.REDEEM_TYPE"], function(totas) {
			if (totas) {
        ...
        $scope.noCallCustQuery = function () {
        	...
        	$scope.sendRecv("SOT1630", "query", "com.systex.jbranch.app.server.fps.sot1630.SOT1630InputVO", $scope.inputVO,
					function(tota, isError) {
						...
						if (!isError) {
							if (tota[0].body.mainList.length == 0) {
								...
								$rootScope.menuItemInfo.url = "assets/txn/SOT1630/SOT1630.html";	
								return;
							}
		...
		$scope.delCar = function(row) {
			...
			$confirm({text: txtMsg}, {size: 'sm'}).then(function() {
				$scope.sendRecv("SOT1630", "delProd", "com.systex.jbranch.app.server.fps.sot1630.SOT1630InputVO", {tradeSEQ : $scope.inputVO.tradeSEQ},
						function(tota, isError) {
							if (!isError) {
		...
		//傳送OP交易  ; 產生動態鎖利贖回申請書表單
		$scope.do_goOP = function () {
			//進行傳送OP交易及列印表單鍵  (批號產生)
			$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': '8', 'tradeType':2 ,
																												   'tradeSeq': $scope.inputVO.tradeSEQ}, 
			function(tota, isError) {
				if (!isError) {
					$scope.sendRecv("SOT1630", "goOP", "com.systex.jbranch.app.server.fps.sot1630.SOT1630InputVO", $scope.inputVO,
							function(tota, isError) {
								if (!isError) {
