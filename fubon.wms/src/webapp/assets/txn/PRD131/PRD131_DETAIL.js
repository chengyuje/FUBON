 ...
 */
'use strict';
eSoafApp.controller('PRD131_DETAILController',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PRD131_DETAILController";
		
		$scope.nowDate = new Date();
		
		// -------------------------------------INFO----------------------------------------
		$scope.sendRecv("PRD131", "getBondInfo", "com.systex.jbranch.app.server.fps.prd131.PRD131InputVO", {'prd_id':$scope.row.PRD_ID},
        ...
        $scope.getBondPrice = function() {
        	$scope.sendRecv("PRD131", "getBondPrice", "com.systex.jbranch.app.server.fps.prd131.PRD131InputVO", $scope.inputVO,
					function(tota, isError) {
						if (!isError) {
		...
		// -------------------------------------DIVIDEND----------------------------------------
		$scope.sendRecv("PRD131", "getBondDividend", "com.systex.jbranch.app.server.fps.prd131.PRD131InputVO", {'prd_id':$scope.row.PRD_ID},
				function(tota, isError) {
					if (!isError) {
		...
		// -------------------------------------交易明細----------------------------------------
		//TYPE: 'BOND' or 'REPO'
		$scope.sendRecv("PRD131", "getBondTrans", "com.systex.jbranch.app.server.fps.prd131.PRD131InputVO", {'prd_id':$scope.row.PRD_ID, 'type':$scope.row.TYPE, 'cust_id':$scope.row.CUST_ID},
				function(tota, isError) {
					if (!isError) {
						debugger
						$scope.trans = tota[0].body.resultList;
						$scope.transVO = tota[0].body;
						return;
					}
		});
