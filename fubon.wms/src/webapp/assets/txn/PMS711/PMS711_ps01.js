'use strict';
eSoafApp.controller('PMS711_ps01Controller', function($rootScope, $scope,
		$controller, $confirm, socketService, ngDialog, projInfoService) {
	$controller('BaseController', {
		$scope : $scope
	});
	$scope.controllerName = "PMS711_ps01Controller";
	//初始化
	$scope.init = function(){
		$scope.inputVO = {
				date_year  : '',
	...
	$scope.query = function() {
		...
		$scope.sendRecv("PMS711","queryPs01","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
		...
		$scope.inputVO.personType = $scope.personType;
		$scope.inputVO.flag = $scope.flag;
		$scope.sendRecv("PMS711", "queryps01","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",
    ...
    $scope.changeToCklcmb = function () {
    	$scope.flag = '1';
    	$("#ps01_flag0").addClass('table-span2');
    	$("#ps01_flag1").removeClass('table-span2');
    	$scope.query();
    }
    $scope.changeToTbsymb = function () {
    	$scope.flag = '2';
    	$("#ps01_flag1").addClass('table-span2');
    	$("#ps01_flag0").removeClass('table-span2');
    	$scope.query();
    } 
	...
	/*$scope.save = function() {
		...
		$scope.sendRecv("PMS711", "saveps01","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",
			$scope.inputVO, function(tota, isError) {
				if (!isError) {
