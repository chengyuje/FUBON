 ...
 */
'use strict';
eSoafApp.controller('PMS711_newGoalController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PMS711_newGoalController";
		$scope.select = function() {
			$scope.inputVO.date_year = $scope.date_year;
			$scope.inputVO.personType = $scope.personType;
			$scope.sendRecv("PMS711","queryNewGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
		...
		$scope.update = function() {
			...
			for(var i=0;i<$scope.inputVO.showList.length;i++){
				if(String($scope.inputVO.showList[i].CARD_GOAL)==''){
					$scope.showMsg("ehl_01_common_022");
					return;
				}
			}
			$scope.sendRecv("PMS711","saveNewGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
				function(tota, isError) {
					if (!isError) {
