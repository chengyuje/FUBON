 ...
 */
'use strict';
eSoafApp.controller('PMS711_newCustGoalController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PMS711_newCustGoalController";
		$scope.select = function() {
			$scope.inputVO.date_year = $scope.date_year;
			$scope.inputVO.personType = $scope.personType;
			$scope.sendRecv("PMS711","queryNewCustGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
		...
		$scope.update = function() {
			...
			for(var i=0;i<$scope.inputVO.showList.length;i++){
				if(String($scope.inputVO.showList[i].M_GOAL)==''){
					$scope.showErrorMsg("ehl_01_common_022");
					return;
				}
			}
			$scope.sendRecv("PMS711","saveNewCustGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
				function(tota, isError) {
					if (!isError) {
