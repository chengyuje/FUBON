...
'use strict';
eSoafApp.controller('CAM210_5_ao_Controller', function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter) {
	$controller('BaseController', {$scope: $scope});
	$scope.controllerName = "CAM210_5_ao_Controller";
	...
	$scope.inquire = function(){
		$scope.sendRecv("CAM210", "expiredActivities_aoList", "com.systex.jbranch.app.server.fps.cam210.CAM210InputVO", $scope.inputVO, function(tota, isError) {
			if (!isError) {
    ...
    $scope.$on("CAM210_5_ao_Controller.txtAO_5", function(event, row) {
    	$scope.txtAO_5(row);
    });
	...
	$scope.$on("CAM210_5_ao_Controller.inquireInit", function() {
		$scope.inquireInit();
    });
