 ...
 */
'use strict';
eSoafApp.controller('CAM210_3_Controller',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "CAM210_3_Controller";
		$scope.row = $scope.row || {};	
		$scope.init = function(){
			$scope.inputVO = {
        ...
        $scope.getList = function(){
        	...
        	$scope.sendRecv("CAM210", "queryToDoListData", "com.systex.jbranch.app.server.fps.cam210.CAM210InputVO",  $scope.inputVO,
        ...
        $scope.action = function(row) {
        	if($scope.inputVO.channel != ''){
        		...
        		$scope.sendRecv("CAM210", "queryAoData_3", "com.systex.jbranch.app.server.fps.cam210.CAM210InputVO",  $scope.inputVO,
        				function(totas, isError) {
        					if (!isError) {
        						if(totas[0].body.resultList.length == 0) {
        							$scope.showMsg("ehl_01_common_009");
    	...
    	$scope.upDataList = function(row) {
    		if($scope.inputVO.channel == '') {
    			...
    			$scope.sendRecv("CAM210", "updLead", "com.systex.jbranch.app.server.fps.cam210.CAM210InputVO",  $scope.inputVO,
    					function(totas, isError) {	
    				if (isError) {
...
//	   	$scope.mappingSet['STYLE'].push({LABEL: '換Code經營', DATA: 'changCode'}); 
     	...
     	$scope.invalidList = function(row) {
    		$scope.sendRecv("CAM210", "invalidLead", "com.systex.jbranch.app.server.fps.cam210.CAM210InputVO",  $scope.inputVO,
    				function(totas, isError) {
	    		if (isError) {
						$scope.showErrorMsg(tota[0].body.MsgData);
				}
