...
eSoafApp.controller('PRD130Controller',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $filter, getParameter, $q, validateService) {
		...
		$scope.init = function() {
			...
			$scope.inputVO.prodType='3';  //3：海外債
		...
		$scope.doSave = function(row) {
			...
			$scope.sendRecv("SOT701", "getCustObuFlag", "com.systex.jbranch.app.server.fps.sot701.SOT701InputVO", { "custID": $scope.inputVO.cust_id },
				function(tota, isError) {
					if (!isError) {
						if ("Y" != tota[0].body) {
...
//						debugger
		...
		$scope.getCustInfo = function(prdId) {
			var deferred = $q.defer();
			$scope.sendRecv("PRD130", "getCustInfo", "com.systex.jbranch.app.server.fps.prd130.PRD130InputVO", {cust_id: $scope.inputVO.cust_id, prd_id: prdId},
				function(totas, isError) {
					if (!isError) {
