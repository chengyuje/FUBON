 ...
 */
'use strict';
eSoafApp.controller('SOT1651Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, $interval, getParameter, $timeout) {
		...
		$scope.controllerName = "SOT1651Controller";
		
		getParameter.XML(["SOT.CUST_TYPE", "SOT.CHANGE_TRADE_SUB_TYPE", "SOT.TRADE_DATE_TYPE", "COMMON.YES_NO", "SOT.TRUST_CURR_TYPE", "SOT.SPEC_CUSTOMER", "SOT.DEDUCTION_STATUS_DYNA", "SOT.PROSPECTUS_TYPE"], function(totas) {
			if (totas) {
        ...
        $scope.noCallCustQuery = function () {
        	...
        	$scope.sendRecv("SOT1650", "query", "com.systex.jbranch.app.server.fps.sot1650.SOT1650InputVO", $scope.inputVO,
					function(tota, isError) {
						...
						if (!isError) {
							if (tota[0].body.mainList.length == 0) {
								...
								$rootScope.menuItemInfo.url = "assets/txn/SOT1650/SOT1650.html";	
								return;
							}
		...
		$scope.delCar = function(row) {
			...
			$confirm({text: txtMsg}, {size: 'sm'}).then(function() {
				$scope.sendRecv("SOT1650", "delProd", "com.systex.jbranch.app.server.fps.sot1650.SOT1650InputVO", {tradeSEQ : $scope.inputVO.tradeSEQ},
						function(tota, isError) {
							if (!isError) {
		...
		$scope.do_goOP = function () {
			...
			//進行傳送OP交易及列印表單鍵  (批號產生)
			$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': '8', 'tradeType':4 ,
																												   'tradeSeq': $scope.inputVO.tradeSEQ}, 
			function(tota, isError) {
				if (!isError) {
					$scope.sendRecv("SOT1650", "goOP", "com.systex.jbranch.app.server.fps.sot1650.SOT1650InputVO", $scope.inputVO,
							function(tota, isError) {
								if (!isError) {
