...
eSoafApp.controller("InitController", ['$rootScope', '$scope', '$controller', 'projInfoService', 'socketService', 'ngDialog', '$location', '$interpolate', '$timeout', '$confirm', 'getParameter', 'sysInfoService',
	function($rootScope, $scope, $controller, projInfoService, socketService, ngDialog, $location, $interpolate, $timeout, $confirm, getParameter, sysInfoService){
		$controller('CMFPG000Controller', {$scope: $scope});
...
//		console.log('WATERMARK='+JSON.stringify($scope.WATERMARK));
		...
		/*-----------------------------------------------------------------------------
		 * @Description: JSB 日盛員工富盛案進行登入測試
		 *-----------------------------------------------------------------------------*/
		$scope.tempUserInfo = sysInfoService.getTempUserInfo();

		$scope.tempUserLogoutConfirm = function() {
			$confirm({
				title: '系統通知',
				text: '親愛的 ' +  $scope.tempUserInfo.EMP_NAME + ' 您好：' +
				 '系統目前尚未開放使用，頁面將進入唯讀模式，可直接按「確定」進行登出，或稍後再進行，謝謝！'
			}, {size: 'sm'})
				.then(() => {
					$scope.sendRecv("CMFPG000", "tlroff", "com.systex.jbranch.app.server.fps.cmfpg000.LoginPageVO", {},
						function (tota, isError) {
							if (isError)
								...
								projInfoService.setTempUserInfo(null);
		...
		if ($scope.tempUserInfo) {
			$scope.tempUserLogoutConfirm();
		}
