...
public class SOT610 extends FubonWmsBizLogic {
	...
	private StringBuffer tradeSQL(SOT610InputVO inputVO, QueryConditionIF queryCondition) {
		...
		sb.append("    UNION ");
		sb.append("    SELECT NULL, TRADE_SEQ, PROD_ID, PROD_NAME, TRADE_DATE, BATCH_SEQ FROM TBSOT_NF_PURCHASE_DYNA "); //基金動態鎖利_申購
		sb.append("    UNION ");
		sb.append("    SELECT NULL, TRADE_SEQ, PROD_ID, PROD_NAME, TRADE_DATE, BATCH_SEQ FROM TBSOT_NF_RAISE_AMT_DYNA "); //基金動態鎖利_母基金加碼
		sb.append("    UNION ");
		sb.append("    SELECT NULL, TRADE_SEQ, PROD_ID, PROD_NAME, TRADE_DATE, BATCH_SEQ FROM TBSOT_NF_REDEEM_DYNA "); //基金動態鎖利_贖回
		sb.append("    UNION ");
		sb.append("    SELECT NULL, TRADE_SEQ, PROD_ID, PROD_NAME, TRADE_DATE, BATCH_SEQ FROM TBSOT_NF_TRANSFER_DYNA "); //基金動態鎖利_轉換
		sb.append("    UNION ");
		sb.append("    SELECT NULL, TRADE_SEQ, PROD_ID, PROD_NAME, TRADE_DATE, '' AS BATCH_SEQ FROM TBSOT_NF_CHANGE_DYNA "); //基金動態鎖利_事件變更
		sb.append("    UNION ");
	...
	public void query(Object body, IPrimitiveMap header) throws JBranchException {
		...
		sb.append("       CASE WHEN T_MAIN.TRADE_STATUS = '3' THEN (SELECT PARAM_NAME FROM TBSYSPARAMETER WHERE PARAM_TYPE = 'SOT.PRINT_REPORT' AND PARAM_CODE = T_MAIN.PROD_TYPE||(CASE WHEN T_MAIN.PROD_TYPE IN ('1', '8') THEN T_MAIN.TRADE_TYPE ELSE '' END)) ");
