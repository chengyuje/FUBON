...
@Component("mao111")
@Scope("request")
public class MAO111 extends FubonWmsBizLogic {
	private DataAccessManager dam = null;
	private Logger logger = LoggerFactory.getLogger(MAO111.class);
	
	public void inquire(Object body, IPrimitiveMap header) throws JBranchException {
		MAO111InputVO inputVO = (MAO111InputVO) body;
		MAO111OutputVO outputVO = new MAO111OutputVO();
		...
		sql.append("  SELECT BASE.SEQ, BASE.DEV_NBR, BASE.DC_NBR, BASE.OP_NBR, BASE.BRA_NBR, BASE.DEV_TAKE_EMP, BASE.DEV_TAKE_EMP_NAME, BASE.DEV_SITE_TYPE, BASE.O_TYPE, ");
		...
		sql.append("  FROM ( ");
		sql.append("    SELECT M.SEQ, M.DEV_NBR, M.DC_NBR, M.OP_NBR, M.BRA_NBR, M.DEV_TAKE_EMP, MEM.EMP_NAME AS DEV_TAKE_EMP_NAME, M.DEV_SITE_TYPE, T_LIST.O_TYPE, ");
		...
		sql.append("        SELECT PARAM_ORDER AS O_TYPE, PARAM_CODE AS START_TIME, PARAM_DESC AS END_TIME ");
		sql.append("        FROM TBSYSPARAMETER ");
		sql.append("        WHERE PARAM_TYPE = 'MAO.USE_PERIOD_HOUR' ");
		sql.append("    ) T_LIST ON 1 = 1 ");
		sql.append("    WHERE M.BRA_NBR = :branchNbr ");
	...
	public void add_query(Object body, IPrimitiveMap header) throws JBranchException {
		...
		sql.append(") ");
		sql.append("SELECT C.CUST_NAME, C.CUST_ID, REL_TYPE, '' AS REL_TYPE_OTH, C.BRA_NBR, C.AO_CODE, BASE.EMP_NAME, R.CUST_ID_M, RC.CUST_NAME AS JOIN_SRV_CUST_NAME ");
		...
		queryCondition.setQueryString(sql.toString());
		queryCondition.setObject("aoCode", inputVO.getAo_code());//getUserVariable(FubonSystemVariableConsts.LOGINID)
		if (!StringUtil.isBlank(inputVO.getCust_id()))
		...
		sql.append("SELECT PARAM_CODE FROM TBSYSPARAMETER WHERE PARAM_TYPE = 'MAO.AOUSER_QRY_MAX_LIMIT' ");
		queryCondition.setQueryString(sql.toString());
		List<Map<String, Object>> limitList = dam.exeQuery(queryCondition);
		Integer limit = Integer.valueOf(ObjectUtils.toString(limitList.get(0).get("PARAM_CODE")));
		if(list.size() > limit)
	...
	public void application(Object body, IPrimitiveMap header) throws JBranchException {
		...
		if(inputVO.getRelationshipList().size() != 0){
			// check 關係戶 2017/4/10
			sql.append("SELECT RC.CUST_NAME, RC.CUST_ID, R.REL_TYPE, R.REL_TYPE_OTH, RC.BRA_NBR, RC.AO_CODE, BASE.EMP_NAME, R.JOIN_SRV_CUST_ID, RC.CUST_NAME AS JOIN_SRV_CUST_NAME ");
			sql.append("FROM TBCRM_CUST_MAST C ");
			sql.append("LEFT JOIN ( ");
			sql.append("  SELECT MEM.EMP_ID, MEM.EMP_NAME, AO.AO_CODE ");
			sql.append("  FROM TBORG_SALES_AOCODE AO ");
		...
		sql.append("SELECT MIN(PARAM_CODE) AS PARAM_CODE, CASE WHEN MAX(PARAM_ORDER) > 10 THEN MIN(PARAM_DESC) ELSE MAX(PARAM_DESC) END AS PARAM_DESC ");
		sql.append("FROM TBSYSPARAMETER ");
		sql.append("WHERE PARAM_TYPE = 'MAO.USE_PERIOD_HOUR' ");
	...
	public void get_emp_id(Object body, IPrimitiveMap header) throws JBranchException {
		...
		List<Map<String,Object>> empIdList = exeQueryForQcf(genDefaultQueryConditionIF()
				.setQueryString(new StringBuilder()
				.append(" SELECT EMP_ID FROM VWORG_AO_INFO ")
				.append(" WHERE AO_CODE = :ao_code ").toString())
				.setObject("ao_code", inputVO.getAo_code()));
		
		String emp_id = "";
		if (empIdList.size() > 0) {
			emp_id = ObjectUtils.toString(empIdList.get(0).get("EMP_ID"));
		} else {
			emp_id = (String) getUserVariable(FubonSystemVariableConsts.LOGINID);
		}
	...
	private List<Map<String, Object>> checkFpsValid(List<String> custList, Date useDate) throws DAOException, JBranchException {
	    if (custList.size() <= 0) return new ArrayList<Map<String, Object>>();
	    ...
	    for (String custID: custList) {
            ...
            try {
                ...
                tmp.put("SIGN_AGMT_YN", StringUtils.isBlank(FP032675Data.getCustTxFlag())
