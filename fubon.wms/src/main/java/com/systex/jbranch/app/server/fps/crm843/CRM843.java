package com.systex.jbranch.app.server.fps.crm843;
...
@Component("crm843")
@Scope("request")
public class CRM843 extends FubonWmsBizLogic {
	...
	public void inquire(Object body, IPrimitiveMap header) throws Exception {
		...
		CRM843OutputVO outputVO = new CRM843OutputVO();
		CRM843InputVO inputVO = (CRM843InputVO) body;
		dam = this.getDataAccessManager();	
		QueryConditionIF queryCondition = dam.getQueryCondition(DataAccessManager.QUERY_LANGUAGE_TYPE_VAR_SQL);		
		StringBuffer sb = new StringBuffer();	
		
		//發送電文
		List<SC120100OutputVO> vos = sc120100Service.searchCredit(inputVO.getCust_id().trim());
		...
		for (SC120100OutputVO sc120100OutputVO : vos) {
			...
			for (SC120100DetailOutputVO data : details) {
				...
				data.setINS_AMT(data.getINS_AMT());
