package com.systex.jbranch.app.server.fps.crm844;
...
@Component("crm844")
@Scope("request")
public class CRM844 extends FubonWmsBizLogic {
	...
	public void inquire(Object body, IPrimitiveMap header) throws Exception {
		...
		CRM844OutputVO outputVO = new CRM844OutputVO();
		CRM844InputVO inputVO = (CRM844InputVO) body;
		dam = this.getDataAccessManager();	
		QueryConditionIF queryCondition = dam.getQueryCondition(DataAccessManager.QUERY_LANGUAGE_TYPE_VAR_SQL);		
		StringBuffer sb = new StringBuffer();	
		
		//發送電文
		List<SC120100OutputVO> vos = sc120100Service.searchStudentLoan(inputVO.getCust_id().trim());
		...
		for (SC120100OutputVO sc120100OutputVO : vos) {
			...
			for (SC120100DetailOutputVO data : details) {
				data.setACT_BAL_NT(cbsservice.amountFormat(data.getACT_BAL_NT()));
				...
				data.setINS_AMT(cbsservice.amountFormat(data.getINS_AMT(), 0));
