...
@Component("org243")
@Scope("request")
public class ORG243 extends FubonWmsBizLogic {
	...
	public void query(Object body, IPrimitiveMap header) throws Exception {
		...
		ORG243InputVO inputVO = (ORG243InputVO) body;
		ORG243OutputVO outputVO = new ORG243OutputVO();
		dam = this.getDataAccessManager();

		QueryConditionIF queryCondition = dam.getQueryCondition(DataAccessManager.QUERY_LANGUAGE_TYPE_VAR_SQL);
		StringBuffer sb = new StringBuffer();

		sb.append("SELECT UHRM.EMP_ID, ");
		sb.append("       UHRM.BRANCH_NBR, ");
		sb.append("       DEFN.BRANCH_NAME, ");
		sb.append("       MEM.EMP_NAME, ");
		sb.append("       UHRM.ISONTHEJOB, ");
		sb.append("       UHRM.MODIFIER || '-' || MOD_MEM.EMP_NAME AS MODIFIER, ");
		sb.append("       UHRM.LASTUPDATE ");
		sb.append("FROM TBORG_UHRM_BRH UHRM ");
		sb.append("LEFT JOIN TBORG_MEMBER MEM ON UHRM.EMP_ID = MEM.EMP_ID ");
		sb.append("LEFT JOIN VWORG_DEFN_INFO DEFN ON UHRM.BRANCH_NBR = DEFN.BRANCH_NBR ");
		sb.append("LEFT JOIN TBORG_MEMBER MOD_MEM ON UHRM.MODIFIER = MOD_MEM.EMP_ID ");
		sb.append("WHERE 1 = 1 ");
	...
	public void getExample(Object body, IPrimitiveMap header) throws JBranchException {
		...
		notifyClientToDownloadFile(url, "私銀理專駐點行上傳資料檔.csv");
	...
	public void add(Object body, IPrimitiveMap header) throws Exception {
		...
		// 整檔清空
		queryCondition.setQueryString("TRUNCATE TABLE TBORG_UHRM_BRH ");
		...
		for (Map<String, Object> dataMap : inputVO.getTEAM_LST()) {
			...
			sb = new StringBuffer();
			sb.append("INSERT INTO TBORG_UHRM_BRH ( ");
			sb.append("  EMP_ID, ");
			sb.append("  BRANCH_NBR, ");
			sb.append("  ISONTHEJOB, ");
			sb.append("  VERSION, ");
			...
			try {
				...
				throw new APException("上傳私銀理專駐點行資料第" + (lineNum) + "筆有誤(人員之駐點行重覆上傳)");
			}
		}
	...
	public void export(Object body, IPrimitiveMap header) throws JBranchException {
		...
		String[] csvHeader = new String[] { "員工編號", "員工姓名", "駐點分行代碼", "駐點分行名稱", "最後修改人", "最後修改時間" };
		...
		if (exportLst.size() > 0) {
			...
			// 將url送回FlexClient存檔
			notifyClientToDownloadFile(url, "私銀理專駐點行資料檔_" + sdfYYYYMMDD.format(new Date()) + ".csv");
		}
