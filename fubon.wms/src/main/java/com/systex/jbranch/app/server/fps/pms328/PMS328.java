...
import com.systex.jbranch.app.server.fps.pms329.PMS329InputVO;
import com.systex.jbranch.app.server.fps.pms329.PMS329OutputVO;
...
//										int cgpr_mass	  = Integer.parseInt(map.get("CGPR_MASS").toString());		// 新等級C (一般存戶-跨優)
	...
	public void queryImage(Object body, IPrimitiveMap header) throws JBranchException {
		...
		//		List<Map<String, Object>> list_AOCODE = inputVO.getList_AOCODE();

		Map<String, Object> map1 = new HashMap<String, Object>();
		try {
			String type_rob = "REGION_CENTER_NAME";
			String type = "TOTAL";
			if ("(客戶權益)".equals(inputVO.getType())) {
				type = "CGPR_SUM";
			}
			if ("(EIP)".equals(inputVO.getType())) {
				type = "EIP_SUM";
			}
			if ("(投保)".equals(inputVO.getType())) {
				type = "II_SUM";
			}
			if ("(TOTAL)".equals(inputVO.getType())) {
				type = "TOTAL";
			}
			//分行
			if ("3".equals(inputVO.getROB())) {
				type_rob = "BRANCH_NBR";
			}
			//區域中心
			if ("1".equals(inputVO.getROB())) {
				type_rob = "REGION_CENTER_ID";
			}
			//員編
			if ("4".equals(inputVO.getROB())) {
				type_rob = "AO_CODE";
			}
			//營運區
			if ("2".equals(inputVO.getROB())) {
