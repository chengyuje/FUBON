 ...
 *
 */
@Component("sot801dyna")
@Scope("request")
public class SOT801DYNA extends SotPdf{
	private DataAccessManager dam = null;
	private Logger logger = LoggerFactory.getLogger(SOT801DYNA.class);

	@Override
	public List<String> printReport() throws Exception {
		String txnCode = "SOT801";
		String reportID = "R4"; //動態鎖利申購表單
		...
		sql.append(" FROM TBSOT_NF_PURCHASE_DYNA D");
		...
		data.addParameter("PROD_M", (String)data_map.get("PROD_ID") + "	" + (String)data_map.get("PROD_CURR") + "	" + (String)data_map.get("PROD_NAME"));
		...
		data.addParameter("PURCHASE_AMT", getBigDecimal(data_map.get("PURCHASE_AMT")));
		data.addParameter("FEE_RATE",  getBigDecimal(data_map.get("FEE_RATE").toString())+"%");
		data.addParameter("FEE", getBigDecimal(data_map.get("FEE")));
		data.addParameter("ENGAGED_ROI", getBigDecimal(data_map.get("ENGAGED_ROI"))+"%");
		data.addParameter("P1_COUPON1", data_map.get("FEE_TYPE").toString().equals("B")? "　生日券　:  Y " :"");
		data.addParameter("P1_TOTAL1", new BigDecimal(data_map.get("PURCHASE_AMT").toString()).add(new BigDecimal(data_map.get("FEE").toString())));
		String transDate = "";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_1")),"Y")) transDate = transDate + "1,";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_4")),"Y")) transDate = transDate + "6,";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_2")),"Y")) transDate = transDate + "11,";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_5")),"Y")) transDate = transDate + "16,";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_3")),"Y")) transDate = transDate + "21,";
		if(StringUtils.equals(ObjectUtils.toString(data_map.get("TRANSFER_DATE_6")),"Y")) transDate = transDate + "26,";
		...
		//子基金1
		data.addParameter("PROD_C1", (String)data_map.get("PROD_ID_C1") + "	" + (String)data_map.get("PROD_CURR_C1") + "	" + (String)data_map.get("PROD_NAME_C1"));
		...
		data.addParameter("PROD_RISK_LV_C1", data_map.get("PROD_RISK_LV_C1").toString());
		data.addParameter("PURCHASE_AMT_C1", getBigDecimal(data_map.get("PURCHASE_AMT_C1")));
		//子基金2
		if(StringUtils.isNotBlank(ObjectUtils.toString(data_map.get("PROD_ID_C2")))) {
			data.addParameter("PROD_C2", (String)data_map.get("PROD_ID_C2") + "	" + (String)data_map.get("PROD_CURR_C2") + "	" + (String)data_map.get("PROD_NAME_C2"));
			...
			data.addParameter("PROD_RISK_LV_C2", data_map.get("PROD_RISK_LV_C2").toString());
			data.addParameter("PURCHASE_AMT_C2", getBigDecimal(data_map.get("PURCHASE_AMT_C2")));
		}
		//子基金3
		if(StringUtils.isNotBlank(ObjectUtils.toString(data_map.get("PROD_ID_C3")))) {
			data.addParameter("PROD_C3", (String)data_map.get("PROD_ID_C3") + "	" + (String)data_map.get("PROD_CURR_C3") + "	" + (String)data_map.get("PROD_NAME_C3"));
			...
			data.addParameter("PROD_RISK_LV_C3", data_map.get("PROD_RISK_LV_C3").toString());
			data.addParameter("PURCHASE_AMT_C3", getBigDecimal(data_map.get("PURCHASE_AMT_C3")));
		}
