 ...
 *
 */
@Repository("nodowngrade")
@Scope("prototype")
public class NODOWNGRADE extends BizLogic {
	...
	public void createFileBth (Object body, IPrimitiveMap<?> header) throws Exception {
		...
		//定義檔案名稱及輸出的jobname
		String  writeFileName="noDowngrade";
		String  attached_name="";
		String  jobName="noDowngrade";
		...
		gft.writeFile(writeFileName, attached_name, (String) SysInfo.getInfoValue(SystemVariableConsts.TEMP_PATH), order, rs, "", false, false);
		rs=gft.getRS(genSql(),s);
		List args = new ArrayList();
		args.add(rs);
		//dam.newTransactionExeMethod(this, "modifyData", args);
		} finally {
    ...
    public StringBuffer genSql()
	{
		...
		sb.append("SELECT RPAD(A.CUST_ID,11,' ')     AS PERSON_ID, ");
		sb.append("RPAD(NVL(B.EMAIL,' '),200,' ')       AS EMAIL, ");
		sb.append("A.VIP_DEGREE as OLDVALUE ");
		sb.append("FROM TBCRM_CUST_NODOWNGRADE A ");
		sb.append("LEFT JOIN TBCRM_CUST_CONTACT B ");
		sb.append("ON A.CUST_ID=B.CUST_ID ");
		sb.append("WHERE TO_CHAR(DATA_DAY,'YYYYMM')=TO_CHAR(SYSDATE,'YYYYMM') ");
