'use strict';
eSoafApp.controller('PMS507Controller', function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $compile, sysInfoService) {
	$controller('BaseController', {$scope: $scope});
	$scope.controllerName = "PMS507Controller";
	...
	$scope.query = function(){
		...
		$scope.sendRecv("PMS507", "query", "com.systex.jbranch.app.server.fps.pms507.PMS507InputVO", $scope.inputVO, function(tota, isError) {
			if (!isError) {
	...
	$scope.getExample = function() {
		$scope.sendRecv("PMS507", "getExample", "com.systex.jbranch.app.server.fps.pms507.PMS507InputVO", $scope.inputVO, function(tota, isError) {
			if (isError) {
	...
	$scope.updFile = function(name, rname) {
		...
		$confirm({text: '是否上傳：' + rname + '?'}, {size: '200px'}).then(function() {
			$scope.sendRecv("PMS507", "updFile", "com.systex.jbranch.app.server.fps.pms507.PMS507InputVO", $scope.inputVO, function(tota, isError) {
				if (isError) {
					$scope.showErrorMsgInDialog(tota.body.msgData);
					return;
				} else {
	...
	$scope.export = function() {
		...
		$scope.sendRecv("PMS507", "export", "com.systex.jbranch.app.server.fps.pms507.PMS507InputVO", $scope.inputVO,
