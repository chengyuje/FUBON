'use strict'; 
eSoafApp.controller('PMS409UController', function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, sysInfoService, getParameter) {
	$controller('BaseController', {$scope: $scope});
	$scope.controllerName = "PMS409UController";
	
	// 繼承
	$controller('PMS409Controller', {$scope: $scope});
	...
	$scope.initPMS409U = function() {
		$scope.isMainten = false;
		$scope.sendRecv("PMS401U", "isMainten", "com.systex.jbranch.app.server.fps.pms401u.PMS401UInputVO", {'itemID': 'PMS409U'}, function(tota, isError) {
			if (!isError) {
