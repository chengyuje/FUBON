 ...
 */
'use strict';
eSoafApp.controller('SOT111Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, $interval, getParameter, $timeout) {
		...
		$scope.controllerName = "SOT111Controller";
		
		getParameter.XML(["SOT.CUST_TYPE", "SOT.CHANGE_TRADE_SUB_TYPE", "SOT.TRADE_DATE_TYPE", "COMMON.YES_NO", "SOT.EBANK_PRTDOC_URL", "SOT.CHG_PRTDOC_URL", "SOT.BARGAIN_STATUS", "SOT.TRUST_CURR_TYPE", "SOT.PROSPECTUS_TYPE","SOT.BARGAIN_STATUS","SOT.SPEC_CUSTOMER"], function(totas) {
			if (totas) {
        ...
        $scope.init = function() {
			...
			$scope.inputVO = {
					...
					isBackendInCart: "N",						//交易中是否其中一筆為後收型基金
							...
							if (tota[0].body.mainList.length == 0) {
								...
								if ($scope.fromFPS) {
									...
									if($scope.inputVO.trustTS == 'S'){
										$scope.setSOTurl('assets/txn/SOT110/SOT110.html');
									}else{
										$scope.setSOTurl('assets/txn/SOT112/SOT112.html');
									}
									...
									if($scope.inputVO.trustTS == 'S'){
										...
										$rootScope.menuItemInfo.url = "assets/txn/SOT112/SOT112.html";
									}
...
//							
							...
							$scope.inputVO.isBackendInCart = tota[0].body.isBackendInCart;	//交易中是否其中一筆為後收型基金
...
//			}
		...
		$scope.do_goOP = function () {
			...
			if ($scope.inputVO.bargainFeeFlag == null || $scope.inputVO.bargainFeeFlag == '2') {
						...
						$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': 'NF', 'tradeType':1 ,
																															   'tradeSeq': $scope.inputVO.tradeSEQ}, 
						function(tota, isError) {
							if (!isError) {
								$scope.sendRecv("SOT110", "goOP", "com.systex.jbranch.app.server.fps.sot110.SOT110InputVO", $scope.inputVO,
										function(tota, isError) {
											if (!isError) {
												if (tota[0].body.errorMsg != '' && tota[0].body.errorMsg != null) {
													...
													$scope.query();
													$scope.printReport();//產生基金單筆申購申請書表單
					...
					if (!isError) {
						if (tota[0].body.errorMsg != '' && tota[0].body.errorMsg != null) {
							...
							$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': 'NF', 'tradeType':1 ,
								...
								function(tota, isError) {
									if (!isError) {
										  $scope.sendRecv("SOT110", "goBANK", "com.systex.jbranch.app.server.fps.sot110.SOT110InputVO", {
												   'tradeSEQ': $scope.inputVO.tradeSEQ},
										  function(tota, isError) {
											 if (!isError) {
												if (tota[0].body.errorMsg != '' && tota[0].body.errorMsg != null) {
		...
		$scope.printReport = function() {
			//查詢是否為"後收型基金"
			$scope.isBackend = '';
			$scope.sendRecv("SOT140", "isBackend", "com.systex.jbranch.app.server.fps.sot140.SOT140InputVO", {'outProdID': $scope.carList[0].PROD_ID},
