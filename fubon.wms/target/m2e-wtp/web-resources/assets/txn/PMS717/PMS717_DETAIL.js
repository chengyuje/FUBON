 ...
 */
'use strict';
eSoafApp.controller('PMS717_DETAILController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $timeout, $filter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PMS717_DETAILController";
	  //import xls
		$scope.importfile = function() 
		{
			$scope.inputVO.prodType = $scope.prodType;
			$scope.inputVO.yearMon = $scope.yearMon;
			$scope.sendRecv("PMS717", "addData", "com.systex.jbranch.app.server.fps.pms717.PMS717InputVO", $scope.inputVO,
	  ...
	  //調用存儲過程存excel表
	   $scope.callStored = function() {
			$scope.sendRecv("PMS717", "callStored", "com.systex.jbranch.app.server.fps.pms717.PMS717InputVO", $scope.inputVO,
					function(tota, isError) {
