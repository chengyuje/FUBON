	...
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $timeout, $filter) {
	  ...
	  //import xls
		$scope.importfile = function() {
			...
			$scope.inputVO.bounty_Type = $scope.bounty_Type;
			if($scope.inputVO.realfileName==null||$scope.inputVO.realfileName==undefined){
				$scope.showErrorMsg('上傳資料不能為空');
				return;
			}
			$scope.sendRecv("PMS710", "addData", "com.systex.jbranch.app.server.fps.pms710.PMS710InputVO", $scope.inputVO,
	  ...
	  //調用存儲過程存excel表
	   $scope.callStored = function() {
			$scope.sendRecv("PMS710", "callStored", "com.systex.jbranch.app.server.fps.pms710.PMS710InputVO", $scope.inputVO,
					function(tota, isError) {
