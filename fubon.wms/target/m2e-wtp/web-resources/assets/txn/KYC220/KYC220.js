 ...
 */
'use strict';
eSoafApp.controller('KYC220Controller',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, sysInfoService,getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "KYC220Controller";
		
        //簽核狀態
		getParameter.XML(["KYC.QUEST_STAUS"],function(totas){
			if(totas){
				//簽核狀態
				$scope.mappingSet['KYC.QUEST_STAUS'] = totas.data[0];
			}
		});
		// date picker
...
//			$scope.submitDate;
		...
		$scope.inquire = function() {
			// alert('pause')
			$scope.sendRecv("KYC220", "inquire", "com.systex.jbranch.app.server.fps.kyc220.KYC220InputVO", $scope.inputVO,
		...
		$scope.check = function(row){
			var dialog = ngDialog.open({
				template: 'assets/txn/KYC220/KYC220_Check.html',
				className: 'KYC220_Check',
