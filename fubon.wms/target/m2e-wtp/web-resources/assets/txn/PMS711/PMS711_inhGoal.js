 ...
 */
'use strict';
eSoafApp.controller('PMS711_inhGoalController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PMS711_inhGoalController";
		$scope.select = function() {
			$scope.inputVO.date_year = $scope.date_year;
			$scope.inputVO.personType = $scope.personType;
			$scope.sendRecv("PMS711","queryInhGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
		...
		$scope.update = function() {
			...
			for(var i=0;i<$scope.inputVO.showList.length;i++){
				if(String($scope.inputVO.showList[i].D_GOAL)=='' || $scope.inputVO.showList[i].D_GOAL==null){
			...
			$scope.sendRecv("PMS711","saveInhGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
				function(tota, isError) {
					if (!isError) {
