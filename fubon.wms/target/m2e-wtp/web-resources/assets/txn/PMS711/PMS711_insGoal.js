 ...
 */
'use strict';
eSoafApp.controller('PMS711_insGoalController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PMS711_insGoalController";
		$scope.select = function() {
			$scope.inputVO.date_year = $scope.date_year;
			$scope.inputVO.personType = $scope.personType;
			$scope.sendRecv("PMS711","queryInsGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
		...
		$scope.update = function() {
			...
			$scope.sendRecv("PMS711","saveInsGoal","com.systex.jbranch.app.server.fps.pms711.PMS711InputVO",$scope.inputVO,
				function(tota, isError) {
					if (!isError) {
