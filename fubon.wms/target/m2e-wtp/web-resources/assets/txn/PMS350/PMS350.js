 ...
 */
'use strict';
eSoafApp.controller('PMS350Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, getParameter,$timeout,sysInfoService,validateService) {
...
//				});
		...
		$scope.NAME_CHANGE = function(){
			$scope.sendRecv("PMS350", "QUERY_NAME_VIEW", "com.systex.jbranch.app.server.fps.pms350.PMS350InputVO", $scope.inputVO,
					function(tota, isError) {
						$scope.mappingSet['RPT_NAME_D'] = [];
						angular.forEach(tota[0].body.namelist, function(row, index, objs){
							$scope.mappingSet['RPT_NAME_D'].push({LABEL: row.RPT_NAME_D, DATA: row.RPT_NAME_D});
		...
		$scope.TYPE_CHANGES = function(){
			$scope.sendRecv("PMS350", "QUERY_TYPE_VIEW", "com.systex.jbranch.app.server.fps.pms350.PMS350InputVO", $scope.inputVO,
					function(tota, isError) {
