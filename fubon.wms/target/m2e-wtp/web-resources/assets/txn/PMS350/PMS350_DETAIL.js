 ...
 */
'use strict';
eSoafApp.controller('PMS350_DETAILController',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter,sysInfoService) {
		...
		console.log($scope.MARQUEE_FLAG);
        ...
        $scope.dateChange = function(){
        	...
        	if($scope.inputVO.sCreDate!=''){
        		$scope.RegionController_getORG($scope.inputVO).then(function(data) {
        			if(sysInfoService.getPriID() == "009" || sysInfoService.getPriID() == "010" 
                		|| sysInfoService.getPriID() == "011" || sysInfoService.getPriID() == "002" || sysInfoService.getPriID() == "003") {
                		$scope.inquireInit();
        	        	$scope.query();
        	        }
        			$scope.disableEmp_id();	
        ...
        $scope.getRPTCol = function(){
			$scope.colList = [];
			$scope.sendRecv("PMS350", "queryRPTCol", "com.systex.jbranch.app.server.fps.pms350.PMS350DetailInputVO", {'seq':rptMastSeq},
        ...
        $scope.getAuthority = function(){
			$scope.authorityList = [];
			$scope.sendRecv("PMS350", "queryAuthority", "com.systex.jbranch.app.server.fps.pms350.PMS350DetailInputVO", {'seq':rptMastSeq},
					function(tota, isError) {
						if (!isError) {
							if(tota[0].body.totalList.length == 0) {
								$scope.showMsg("ehl_01_common_009");
...
//	    		$scope.showErrorMsg('欄位檢核錯誤:資料年月為必輸條件!!');
//        		return;
//        	}
			...
			$scope.sendRecv("PMS350", "queryRPTData", "com.systex.jbranch.app.server.fps.pms350.PMS350DetailInputVO", $scope.inputVO,
				function(tota, isError) {
					if (!isError) {
						if(tota[0].body.totalList.length == 0) {
							$scope.showMsg("ehl_01_common_009");
...
//	alert($scope.csvList.length);		
//alert($scope.csvList[1].aa)			
			
	//alert(JSON.stringify($scope.csvList2))		
			
			
//			angular.forEach($scope.mappingSet, function(row, index, objs){
//				console.log(row.DATA);
//					
//			});		
		  // alert(JSON.stringify($scope.mappingSet[4]))	
			
			
			
			$scope.sendRecv("PMS350", "export","com.systex.jbranch.app.server.fps.pms350.PMS350OutputVO",{'reportId': $scope.DataRow.RPT_NAME + "：" + $scope.DataRow.RPT_EXPLAIN, 'list':$scope.csvList, 'totalList':$scope.colList},
					function(tota, isError) {
						if (isError) {
		...
		$scope.getValue = function(rr){					
			$scope.sendRecv("PMS350", "queryDTLREC", "com.systex.jbranch.app.server.fps.pms350.PMS350DetailInputVO", $scope.inputVO,
					function(tota, isError) {
				...
				angular.forEach(tota[0].body.totalList, function(row, index, objs){
					   ...
					   if(row.ROW_SEQ == rr.ROW_SEQ )
							...
							//$scope.csvList[rr.ROW_SEQ].DATAE=row.CONTENT;     
