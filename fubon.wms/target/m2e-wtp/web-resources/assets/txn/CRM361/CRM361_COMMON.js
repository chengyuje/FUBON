 ...
 */
'use strict';
eSoafApp.controller('CRM361_COMMONController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $q, getParameter, $filter, sysInfoService) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "CRM361_COMMONController";

		// combobox
		getParameter.XML(["CRM.CON_DEGREE", "CRM.VIP_DEGREE", "COMMON.YES_NO", "CRM.TRS_APL_REASON", "CRM.QUERY_TRS_TYPE_1", "FUBONSYS.BMMGR_ROLE"], function(totas) {
			if (totas) {
				...
				$scope.FUBONSYS_BMMGR_ROLE = totas.data[totas.key.indexOf('FUBONSYS.BMMGR_ROLE')];
				
				debugger
				//是否為分行主管角色
				$scope.isBMMGR = false;
                angular.forEach($scope.FUBONSYS_BMMGR_ROLE, function(row){
			...
			$scope.sendRecv("CRM381", "ao_inquire", "com.systex.jbranch.app.server.fps.crm381.CRM381InputVO", {'region_center_id': $scope.inputVO.region_center_id,'branch_area_id': $scope.inputVO.branch_area_id,'branch_nbr': $scope.inputVO2.new_branch_nbr},
					function(tota, isError) {
						if (!isError) {
							...
							angular.forEach(tota[0].body.ao_list, function(row) {
								if(row.TYPE == '1')
	    							$scope.ao_code_common2.push({'LABEL': (row.EMP_NAME+'(主Code)'), 'DATA': row.AO_CODE, 'BRANCH_NBR': row.BRANCH_NBR, 'TYPE': row.TYPE});
	    						else if(row.TYPE == '2')
	    							$scope.ao_code_common2.push({'LABEL': (row.EMP_NAME+'(副Code)'), 'DATA': row.AO_CODE, 'BRANCH_NBR': row.BRANCH_NBR, 'TYPE': row.TYPE});
	    						// mantis 3901 請移除下拉選單有(維護) CODE的選項 //#5649加回來，可移入任何CODE
	    						else if(row.TYPE == '3')
	    							$scope.ao_code_common2.push({'LABEL': (row.EMP_NAME+'(維護Code)'), 'DATA': row.AO_CODE, 'BRANCH_NBR': row.BRANCH_NBR, 'TYPE': row.TYPE});
							});
							
							$scope.ao_code_common2 = $filter('orderBy')($scope.ao_code_common2, 'TYPE', false);
							deferred.resolve();
						}
					}
			);
		...
		$scope.inquire_common = function() {
			...
			debugger
			if($scope.isBMMGR) {
				//分行主管先檢核是否有"調離職理專轄下客戶指派名單"
				$scope.sendRecv("CRM361", "chkInvalidRMPlist", "com.systex.jbranch.app.server.fps.crm361.CRM361InputVO", $scope.inputVO,
						function(tota, isError) {
					debugger
							if (!isError) {
								if(tota[0].body.errorMsg) {
									...
									$scope.sendRecv("CRM361", "inquire_common", "com.systex.jbranch.app.server.fps.crm361.CRM361InputVO", $scope.inputVO,
											function(tota, isError) {
												if (!isError) {
        ...
        $scope.setNewAo = function() {
        	...
        	if($scope.inputVO2.new_ao_code) {
        		...
        		if($filter('filter')($scope.ao_code_common2, {'DATA': $scope.inputVO2.new_ao_code})[0].TYPE == '3') {
...
//        	var checkAoList = Object.keys(groupAo);
//        	var checkAoList = _.chain(data).groupBy('NEW_AO_CODE')
//        	.toPairs().map(function (pair) {
//        		return _.zipObject(['AO_CODE', 'DATA'], pair); 
//        	}).value();
        	...
        	if ($scope.inputVO2.process_type == '3') {
	        	$scope.checkMax().then(function(res_data) {
	        		if (res_data.resultList2 == 'ERR2') {
						...
						$scope.showErrorMsg("客戶:" + res_data.resultList + ", RM輪調後，帶走核心客戶，一年內不得再帶走該RM轄下原分行客戶");
