...
eSoafApp.controller('PRD240_SORTController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, getParameter, $filter) {
		...
		$scope.init = function() {
			...
			$scope.sendRecv("PRD240", "etf_getRank", "com.systex.jbranch.app.server.fps.prd240.PRD240InputVO", $scope.inputVO,
					...
					if (!isError) {
						...
						angular.forEach(tota[0].body.resultList, function(row) {
							...
							if(exiIndex != -1) {
								...
								$scope.listBase[exiIndex].old_etf_name = row.ETF_CNAME;
						...
						angular.forEach(tota[0].body.resultList2, function(row) {
							...
							if(exiIndex != -1) {
								...
								$scope.listBase[exiIndex].etf_name = row.ETF_CNAME;
		...
		$scope.checkID = function(dataRow) {
			if(dataRow.prd_id) {
				$scope.sendRecv("PRD240", "etf_checkID", "com.systex.jbranch.app.server.fps.prd240.PRD240InputVO", {'prd_id':dataRow.prd_id, 'status':'S'},
					function(tota, isError) {
						if (!isError) {
							if (tota[0].body.errorMsg != undefined) {
								...
								dataRow.etf_name = tota[0].body.etf_name;
		...
		$scope.goPRD120_ETF = function (dataRow) {
...
//				}
		...
		$scope.reallySave = function(data) {
			$scope.sendRecv("PRD240", "etf_saveSort", "com.systex.jbranch.app.server.fps.prd240.PRD240InputVO", {'review_list': data,'date': $scope.inputVO.date},
				function(tota, isError) {
					if (!isError) {
