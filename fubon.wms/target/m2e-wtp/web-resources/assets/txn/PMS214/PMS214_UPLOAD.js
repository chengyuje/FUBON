...
eSoafApp.controller('PMS214_UPLOADController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $timeout, $filter) {
	  ...
	  //import xls
		$scope.importfile = function() {
			...
			$scope.inputVO.AOCODE   = $scope.AOCODE;
			if($scope.inputVO.realfileName==null||$scope.inputVO.realfileName==undefined){
				$scope.showErrorMsg('上傳資料不能為空');
				return;
			}
			$scope.sendRecv("PMS214", "importData", "com.systex.jbranch.app.server.fps.pms214.PMS214InputVO", $scope.inputVO,
    ...
    //調用存儲過程存excel表
	   $scope.callStored = function() {
			$scope.sendRecv("PMS214", "callStored", "com.systex.jbranch.app.server.fps.pms214.PMS214InputVO", $scope.inputVO,
					function(tota, isError) {
