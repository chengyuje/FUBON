 ...
 */
'use strict';
eSoafApp.controller('PRD210_TARGETController',
	function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "PRD210_TARGETController";
		
		// combobox
		getParameter.XML(["FPS.STOCK_BOND_TYPE"], function(totas) {
			if (totas) {
		...
		if(projInfoService.getAuthorities().MODULEID[projInfoService.$currentModule].ITEMID["PRD210_TARGET"])
			$scope.CanConfirm = projInfoService.getAuthorities().MODULEID[projInfoService.$currentModule].ITEMID["PRD210_TARGET"].FUNCTIONID["confirm"];
		else
		...
		$scope.inquire = function() {
			...
			if($scope.inputVO.target_id)
				$scope.inputVO.target_id = $scope.inputVO.target_id.toUpperCase();
			$scope.sendRecv("PRD210", "inquireTarget", "com.systex.jbranch.app.server.fps.prd210.PRD210InputVO", $scope.inputVO,
		...
		$scope.download = function(row) {
			$scope.sendRecv("PRD210", "downloadTarget", "com.systex.jbranch.app.server.fps.prd210.PRD210InputVO", $scope.inputVO,
		...
		$scope.upload = function() {
			var dialog = ngDialog.open({
				template: 'assets/txn/PRD210/PRD210_UPLOAD_TARGET.html',
				className: 'PRD210',
				showClose: false,
		...
		$scope.review = function (status) {
			...
			$confirm({text: '是否' + ((status == 'N') ? '退回' : '核可'),title:'覆核確認'}, {size: 'sm'}).then(function() {
				$scope.sendRecv("PRD210", "reviewTarget", "com.systex.jbranch.app.server.fps.prd210.PRD210InputVO", {'review_list': ans,'status': status},
						function(tota, isError) {
							if (isError) {
			...
			if(row.cmbAction) {
				if(row.cmbAction == "D") {
					$confirm({text: '是否刪除此筆資料!!'}, {size: 'sm'}).then(function() {
						$scope.sendRecv("PRD210", "deleteTarget", "com.systex.jbranch.app.server.fps.prd210.PRD210InputVO", {'target_id': row.TARGET_ID},
                				function(totas, isError) {
                                	if (isError) {
		...
		$scope.edit = function (row) {
			var dialog = ngDialog.open({
				template: 'assets/txn/PRD210/PRD210_MANTAIN.html', 
				className: 'PRD210',
				showClose: false,
                controller: ['$scope', function($scope) {
                	$scope.row = row;
