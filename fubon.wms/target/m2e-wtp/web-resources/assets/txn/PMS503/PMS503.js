'use strict';
eSoafApp.controller('PMS503Controller', function($rootScope, $scope, $controller, $confirm, socketService, ngDialog, projInfoService, $compile, sysInfoService) {
	$controller('BaseController', {$scope: $scope});
	$scope.controllerName = "PMS503Controller";
	...
	$scope.query = function(){
		...
		$scope.sendRecv("PMS503", "query", "com.systex.jbranch.app.server.fps.pms503.PMS503InputVO", $scope.inputVO, function(tota, isError) {
			if (!isError) {
	...
	$scope.getExample = function() {
		$scope.sendRecv("PMS503", "getExample", "com.systex.jbranch.app.server.fps.pms503.PMS503InputVO", $scope.inputVO, function(tota, isError) {
			if (isError) {
	...
	$scope.updFile = function(name, rname) {
		...
		$confirm({text: '是否上傳：' + rname + '?'}, {size: '200px'}).then(function() {
			$scope.sendRecv("PMS503", "updFile", "com.systex.jbranch.app.server.fps.pms503.PMS503InputVO", $scope.inputVO, function(tota, isError) {
				if (isError) {
					$scope.showErrorMsgInDialog(tota.body.msgData);
					return;
				} else {
	...
	$scope.export = function() {
		...
		$scope.sendRecv("PMS503", "export", "com.systex.jbranch.app.server.fps.pms503.PMS503InputVO", $scope.inputVO,
