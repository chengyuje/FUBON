'use strict';
eSoafApp.controller('FPS910_SPP_BONDSController',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "FPS910_SPP_BONDSController";
				...
				function(tota, isError) {
					if (!isError) {
						...
						if(tota[0].body.resultList.length == 0) {
							...
							$scope.inputVO.bondsList.push({'CUST_RISK_ATR': 'C1-1','CIRCLE': 1,'SUBITEM': [{'INV_PRD_TYPE': '3','CIRCLE': 2,'SUBITEM': []}]}
...
//            	$scope.showSuccessMsg('刪除成功');
		...
		$scope.$on('getSppResultList', function(){
			$scope.connector('set', 'sppBondsList', $scope.inputVO.bondsList);
		});
});