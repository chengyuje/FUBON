'use strict';
eSoafApp.controller('INS930Controller',
	function($scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, getParameter) {
		$controller('BaseController', {$scope: $scope});
		$scope.controllerName = "INS930Controller";
		
		// combobox
		getParameter.XML(["INS.WARD_TYPE", "INS.HOSPITAL_TYPE", "INS.PARA_HEADER_STATUS", "INS.UNIT"], function(totas) {
			if (totas) {
				$scope.WARD_TYPE = totas.data[totas.key.indexOf('INS.WARD_TYPE')];
				$scope.HOSPITAL_TYPE = totas.data[totas.key.indexOf('INS.HOSPITAL_TYPE')];
		...
		// 判斷主管直接根據有無覆核權限
		if(projInfoService.getAuthorities().MODULEID[projInfoService.$currentModule].ITEMID["INS930"])
			$scope.CanConfirm = projInfoService.getAuthorities().MODULEID[projInfoService.$currentModule].ITEMID["INS930"].FUNCTIONID["confirm"];
		else
		...
		$scope.init = function() {
			...
			$scope.sendRecv("INS930", "initial", "com.systex.jbranch.app.server.fps.ins930.INS930InputVO", {},
				function(totas, isError) {
                	if (!isError) {
                		...
                		$scope.inputVO.hospitalList = totas[0].body.hospitalList;
        ...
        $scope.goReview = function() {
        	...
        	//
        	$scope.sendRecv("INS930", "goReview", "com.systex.jbranch.app.server.fps.ins930.INS930InputVO", $scope.inputVO,
				function(totas, isError) {
                	if (!isError) {
        ...
        $scope.review = function (status) {
			$confirm({text: '是否' + ((status == 'R') ? '退回' : '核可')}, {size: 'sm'}).then(function() {
				$scope.sendRecv("INS930", "review", "com.systex.jbranch.app.server.fps.ins930.INS930InputVO", {'para_no': $scope.inputVO.para_no, 'status': status},
					function(totas, isError) {
	                	if (!isError) {
