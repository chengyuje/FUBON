var appObj2jsObj = function (obj) {
	...
	return {
		...
		STOCK_BOND_TYPE: (obj.product.categoryName === '債券型') ? 'B' : 'S',
...
function formatWebViewParam() {
  ...
  ['transaction', 'param', 'mixedParam'].forEach(function (key) {
    that.getScope()[key + 'List'] = that.getScope()[key + 'List'].map(function (item) {
      return appObj2jsObj(item);
    });
  });
  
  that.getScope().paramListStock = that.getScope().paramList
  that.getScope().paramList = that.getScope().mixedParamList
