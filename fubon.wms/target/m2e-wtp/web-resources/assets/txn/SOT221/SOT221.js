 ...
 */
'use strict';
eSoafApp.controller('SOT221Controller',
	function($rootScope, $scope, $controller, socketService, ngDialog, projInfoService, $q, $confirm, $filter, $interval, getParameter, $timeout) {
		...
		$scope.controllerName = "SOT221Controller";
					...
					function(tota, isError) {
						if (!isError) {
							if (tota[0].body.mainList.length == 0) {
								...
								if ($scope.fromFPS) {
									// from FPS_SOT.js
									$scope.setSOTurl('assets/txn/SOT220/SOT220.html');
								} else if($scope.inputVO.trustTS == "M") {
									$rootScope.menuItemInfo.url = "assets/txn/SOT223/SOT223.html";
								} else {
									$rootScope.menuItemInfo.url = "assets/txn/SOT220/SOT220.html";
								}
								return;
							}
							...
							$scope.isRecNeeded =tota[0].body.recNeeded;  //是否需錄音序號
				...
				$scope.sendRecv("SOT220", "query", "com.systex.jbranch.app.server.fps.sot220.SOT220InputVO", $scope.inputVO,
						function(tota, isError) {
							if (!isError) {
								...
								$scope.cartList = tota[0].body.carList;//SOT221.html用cartList而非carList注意
								$scope.setApplyBargainList(tota[0].body.carList);//設定議價狀態清單
								deferred.resolve("success");
							}
				});
...
//	    }, 15000); //15000 =15秒
		...
		$scope.getSOTCustInfoCT = function () {
			$scope.sendRecv("SOT220", "getSOTCustInfoCT", "com.systex.jbranch.app.server.fps.sot220.SOT220InputVO", {'custID':$scope.inputVO.custID, 'prodType':2, 'tradeType':2, 'trustTS':$scope.inputVO.trustTS},
...
//							$scope.recSeqFlag = $scope.recSeqFlagOrg;
		...
		$scope.goPage = function(row) {debugger;
...
//		};
		...
		$scope.do_goOP = function () {
			//$scope.noCallCustQuery();
//			if($scope.isRecNeeded && !$scope.inputVO.recSEQ){
//				$scope.showErrorMsg("ehl_02_common_007");
//				return;
//			}
			if ($scope.inputVO.bargainFeeFlag == null || $scope.inputVO.bargainFeeFlag == '2') {
				...
				$scope.sendRecv("SOT712", "updateBatchSeq", "com.systex.jbranch.app.server.fps.sot712.SOT712InputVO", {'prodType': 'ETF', 
					...
					function(tota, isError) {
						if (!isError) {
							$scope.sendRecv("SOT220", "goOP", "com.systex.jbranch.app.server.fps.sot220.SOT220InputVO", {'custID': $scope.inputVO.custID, 
										 																				 'tradeSEQ': $scope.inputVO.tradeSEQ,
										 																				 'isRecNeeded':$scope.recNeeded,
									   																					 'recSEQ':$scope.inputVO.recSEQ,
